
<p align="center">
 	<a href="https://github.com/johnnymast"><img src="https://img.shields.io/github/followers/johnnymast?style=social" alt="GitHub"></a>
	<a href="https://twitter.com/mastjohnny"><img src="https://img.shields.io/twitter/follow/mastjohnny?style=social" alt="Twitter"></a>
	<a href="https://github.com/sponsors/johnnymast"><img src="https://img.shields.io/github/sponsors/johnnymast?style=socia" alt="Sponsors"></a>
</p>


<!--
<p align="center"> 
  <img src="https://profile-counter.glitch.me/johnnymast/count.svg" loop="none" />
</p>
-->

## Hi, I'm Johnny Mast

**Senior software engineer with over 25 years of experience**, combining deep Linux knowledge with professional backend, systems, and full‑stack development. I enjoy building reliable, maintainable software and turning complex ideas into practical, working solutions.

### 🛠 Skills
- **Programming**: PHP, JavaScript, Node.js, MySQL  
- **Systems & Infrastructure**: Linux internals, systems programming, Docker  
- **Architecture**: software architecture, systems design  
- **DevOps & Automation**: CI/CD, automation tooling  
- **Frontend / Backend**: full‑stack development  
- **Soft skills**: analytical thinking, mentoring, technical leadership  

### 💼 Experience Highlights
- Built and maintained websites, mobile apps, and integrations for various clients.  
- Developed interactive TV portals and early smart‑TV applications for major broadcasters.  
- Served as lead developer at a hosting company, creating internal systems and automated order processing.  
- Led development of a **production‑ready JavaScript game engine**, transforming a partial prototype into a stable, compliant platform used across multiple jurisdictions.  
- Worked with international teams (UK, NL, and more) on online casino platforms and regulatory compliance.

### 🔍 Currently Looking for Work
I'm open to new opportunities in backend development, systems engineering, and technical leadership.  
If you're hiring, feel free to reach out — I'd love to connect.

### ❤️ Support My Work
If you enjoy my open‑source projects or want to support me while I'm between roles, **donations are very welcome**.  
Your support helps me continue building tools and experimenting with new ideas.

### 🔗 Links
- **LinkedIn**: https:\/\/www.linkedin.com\/in\/johnnymast\/  
- **My public GitHub projects**: https:\/\/github.com\/johnnymast?tab=repositories


<br>
<br>
<a href="https://github.com/johnnymast">
	<img src="https://github-readme-stats.vercel.app/api?username=johnnymast&show_icons=true&theme=radical&hide_border=true&hide_title=true" />
	<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=johnnymast&layout=compact&theme=radical&hide_border=true" />
</a>

